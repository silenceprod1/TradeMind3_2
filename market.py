...
#
# Изменения vs 7.2:
# - Добавлен детектор FVG (Fair Value Gap / Imbalance)
#   на 5M и 15M. Возвращается в market["fvgs"].
# - FVG используется как вспомогательный сигнал:
#   если sweep или entry внутри FVG — бонус к score.
# ============================================================
    ...
    for i in range(left, len(candles) - right):
        ...
        if kind == "high":
            ...
            if left_ok and right_ok:
                result.append({"price": price, "index": i,
...
def detect_fvgs(candles, tf, price):
    """
    Ищем незакрытые FVG (Fair Value Gap / имбаланс) на таймфрейме.

    FVG на трёх свечах (c1, c2, c3):
      - Bullish: low[c3] > high[c1] — разрыв вверх
      - Bearish: high[c3] < low[c1] — разрыв вниз

    Зона считается filled, если цена после формирования
    прошла через неё полностью.
    """
...
def format_major_liquidity(data):
    ...
    if not bsl:
        ...
        for i, lvl in enumerate(bsl, 1):
            lines.append(f"{i}. ${lvl['price']:.6f} • {lvl['distance_pct']:.2f}% • S{lvl['strength']:.0f} • {lvl.get('source','?')}")
