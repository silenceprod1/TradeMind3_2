...
def pnl_p_net(e, x, d):
    """Процентный PnL с вычетом fee + slippage."""
    gross = pnl_p(e, x, d)
    return gross - (FEE_PCT + SLIP_PCT)
...
def blend(e, fx, d, p1d, p1e, p1p, p2d, p2e, p2p):
    ...
    # Издержки применяем один раз на всю сделку
...
def sim(trade, c5, start, max_h, cfg):
    ...
    # --- 1. Ждём касания entry (макс 12 часов) ---
